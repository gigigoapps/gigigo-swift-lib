...
- `set` on a store with an `authenticationPolicy` recreates an existing key (delete + add) rather than updating in place, because `kSecAttrAccessControl` is add-only for `SecItemUpdate`. This is what lets you upgrade an existing unprotected key to a policy-gated one — an in-place update would silently leave it readable without the gate. Changing protection therefore replaces the item (brief window where the key is absent if the re-add fails).
