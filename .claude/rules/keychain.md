...
- Applying an `authenticationPolicy` to an existing key recreates the item (delete + add), because `kSecAttrAccessControl` is add-only for `SecItemUpdate`. `set` does this safely: it reads a non-interactive backup first and refuses (throws `Status.interactionNotAllowed`) rather than delete when the item is protected/locked and can't be backed up; if the re-add then fails it restores the backup at a guaranteed-storable protection (`.afterFirstUnlock`, no custom attributes) so the credential is never lost (it survives at the fallback protection, and the thrown error signals the requested protection was not applied). A protected/locked key thus can't be re-protected in place; `remove` it (or retry while unlocked) first.
