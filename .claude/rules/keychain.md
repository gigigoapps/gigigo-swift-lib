...
- `set` updates an existing key's value in place (`SecItemUpdate`) — including refreshing an already access-control-gated credential (the value update triggers the gate's own authentication, as it should). It only recreates the item (delete + add) when adding an access control to a key that doesn't have one yet (an unprotected item + a store with an `authenticationPolicy`), since `kSecAttrAccessControl` is add-only for `SecItemUpdate`. That recreate is safe: it backs up the (readable) value first and refuses to delete if it can't; if the re-add fails it restores the backup at its ORIGINAL accessibility/synchronizable (no access control), so a failed upgrade never loses the credential nor leaves it more exposed.
- Going the other way — a plain store overwriting a key that already has a `kSecAttrAccessControl` — is NOT auto-recreated (a blind delete+add risks the credential, and `SecItemUpdate` can fail with `errSecParam` for unrelated reasons). The update fails and is surfaced; drop protection with an explicit `remove` + `set`.
